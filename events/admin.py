...
def change_status_to_upcoming(queryset):
    ...
    change_status_to_upcoming.short_description = (
        "change selected events status to upcoming"
    )


def change_status_to_current(queryset):
    queryset.update(event_status="current")
    change_status_to_current.short_description = (
        "change selected events status to current"
    )


def change_status_to_finished(queryset):
    queryset.update(event_status="finished")
    change_status_to_finished.short_description = (
        "change selected events status to finished"
    )
...
class EventAdmin(admin.ModelAdmin):
    ...
    actions = [
        change_status_to_upcoming,
        change_status_to_current,
        change_status_to_finished,
    ]
