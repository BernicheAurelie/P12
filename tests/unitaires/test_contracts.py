...
class TestContract(TestSetUp):
    ...
    def test_change_existing_status_for_client_when_contract_is_signed(self):
        ...
        self.assertEqual(response2.status_code, 200)
        second_contract = Contract.objects.get(payment_due=response2.data['result']['payment_due'])
        self.assertEqual(second_contract.client.existing, False)
        response3 = self.client.patch('/contracts/'+str(second_contract.id)+'/', data={
            "client":self.client_1.id, 
            'signed_status': 'false',
            "amount":200,
            # 'payment_due': "2025-01-01T13:39:47.825918Z"
        })
        modified_contract = Contract.objects.get(payment_due="2025-01-01T13:39:47.825918Z")
        self.assertEqual(response3.status_code, 200)
        ...
        response4 = self.client.patch('/contracts/'+str(second_contract.id)+'/', data={
            "client":self.client_1.id, 
            'signed_status': 'true',
            # "amount":500,
            # 'payment_due': "2025-01-01T13:39:47.825918Z"
        ...
        response5 = self.client.patch('/contracts/'+str(second_contract.id)+'/', data={
            "client":self.client_1.id, 
            'signed_status': 'false',
            # "amount":500,
            # 'payment_due': "2025-01-01T13:39:47.825918Z"
    ...
    def test_CRUD_for_technician(self):
        ...
        response=self.client.get('/contracts/')
        self.assertEqual(response.status_code, 200)
        res = self.client.post('/contracts/', data={
            "client":self.client_1.id, 
            'signed_status': 'true',
            "amount":100,
    ...
    def test_CRUD_contract_for_admin(self):
        ...
        self.assertEqual(res.data["message"], "saler_contact is required to create contract for managers or administrators")
        response = self.client.post('/contracts/', data={
            "client":self.client_1.id, 
            'signed_status': 'true',
            "amount":100,
        ...
        self.assertEqual(new_contract.client.existing, True)
        response4 = self.client.patch('/contracts/'+ str(new_contract.id)+ '/', data={
            "client":self.client_1.id, 
            "amount":1000,
            'payment_due': "2024-01-01T13:39:47.825918Z"
        })
